...
app.post('/deletetask',urlencodedParser,function(req,res){
  ...
  pro.updateOne({username:req.session.key},{$pull: { item:req.session.task,date: req.session.date }},function(err,data){
      if(err)
       throw err;
      else
      {
        pro.updateMany({role:"student"},{ $pull: { item:req.session.task,date: req.session.date }},function(err,data){
          if(err)
           throw err;
          else {
            pro.find({username:req.session.key},function(err,data){
 ...
 app.post('/searchtask',urlencodedParser,function(req,res){
   pro.find({username:req.session.key,item:[$regex:req.body.item]},function(err,data){
     if err
     return err;
     res.render('prolist',{prolist:data[0]});
   });
